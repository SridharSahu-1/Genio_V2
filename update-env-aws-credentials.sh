...
# Script to update AWS credentials in .env.production on EC2
# Usage: ./update-env-aws-credentials.sh [access-key-id] [secret-access-key]
...
echo -e "${YELLOW}🔧 Updating AWS credentials in .env.production on EC2...${NC}"
...
if [ -f .env.production ]; then
    ...
    grep "^AWS_ACCESS_KEY_ID=" .env.production | head -c 30
    echo "..."
else
...
echo ""
echo "🔄 Restarting containers with updated credentials..."
...
echo ""
echo "🔍 Verifying credentials in container..."
...
if [[ "\$CONTAINER_KEY" == *"YOUR_AWS"* ]] || [[ "\$CONTAINER_KEY" == "NOT FOUND" ]]; then
    echo "   ❌ Still wrong: \${CONTAINER_KEY:0:20}..."
    exit 1
else
...
echo ""
echo "✅ Credentials updated and containers restarted!"
ENDSSH
